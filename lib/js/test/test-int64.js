...
QUnit.module('Int64');
  ...
  QUnit.test('Int64', function(assert) {
    ...
    assert.ok(EXPECTED_SMALL_INT64.equals(SMALL_INT64));
    assert.ok(EXPECTED_MAX_JS_SAFE_INT64.equals(MAX_JS_SAFE_INT64));
    assert.ok(EXPECTED_MIN_JS_SAFE_INT64.equals(MIN_JS_SAFE_INT64));
    assert.ok(
    ...
    assert.ok(EXPECTED_MAX_SIGNED_INT64.equals(MAX_SIGNED_INT64));
    assert.ok(EXPECTED_MIN_SIGNED_INT64.equals(MIN_SIGNED_INT64));
    assert.equal(
      EXPECTED_SMALL_INT64_AS_NUMBER,
      SMALL_INT64.toNumber()
    );
    assert.equal(
      Number.MAX_SAFE_INTEGER,
      MAX_JS_SAFE_INT64.toNumber()
    );
    assert.equal(
      Number.MIN_SAFE_INTEGER,
      MIN_JS_SAFE_INT64.toNumber()
    );

    for (let i = 0; i < EXPECTED_INT64_LIST.length; ++i) {
      assert.ok(EXPECTED_INT64_LIST[i].equals(INT64_LIST[i]));
    }
